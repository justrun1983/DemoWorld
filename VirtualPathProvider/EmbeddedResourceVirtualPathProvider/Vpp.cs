...
namespace EmbeddedResourceVirtualPathProvider
{
	...
	{
		private readonly IDictionary<string, List<EmbeddedResource>> resources = new Dictionary<string, List<EmbeddedResource>>();
		...
		public EmbeddedResource GetResourceFromVirtualPath( string virtualPath )
		{
			...
			if ( resources.ContainsKey( key ) )
			{
				//var assemblyName = HttpContext.Current.Items[ "WidgetAssemblyName" ].ToString();
			    var assemblyName = "WebPart";
                var resource = resources[ key ].FirstOrDefault( x => x.AssemblyName == assemblyName );
